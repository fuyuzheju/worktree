...
from app.reminder import ReminderService, Reminder
    ...
    def save_set_result(self):
        ...
        if is_active is not None and duetime and message:
            if self.current_reminder == None:
                self.reminder_service.add_reminder(self.node.identity, duetime, message, None, is_active)
            else:
                self.reminder_service.set_reminder(self.current_reminder.reminder_id, duetime, message, is_active)
        self.accept()
...
class RemindersDialog(QDialog):
    ...
    def __init__(self,
        ...
        self.loader = loader
        self.reminder_service = reminder_service
        self.reminder_service.edited.connect(self.refresh)
    ...
    def set_reminder(self, row_position):
        reminder = self.reminder_service.get_reminder_by_id(self.uid_list[row_position])
        assert reminder is not None
        node_id = reminder.node_id
        node = self.loader.tree.get_node_by_id(node_id)
        assert node is not None
        current_id = self.uid_list[row_position]
        current_reminder = self.reminder_service.get_reminder_by_id(current_id)
        set_dialog = SetReminderDialog(node, self.reminder_service, current_reminder)
        ret = set_dialog.exec_()

    def refresh(self):
        self.reminder_table.setRowCount(0)
        self.uid_list = []
        for reminder_event in self.reminder_service.list_reminders(): 
    ...
    def update_active_status(self, data_store_index, state):
        if 0 <= data_store_index < len(self.reminder_service.list_reminders()):
            self.reminder_service.set_reminder(self.uid_list[data_store_index], active=(state==Qt.Checked))