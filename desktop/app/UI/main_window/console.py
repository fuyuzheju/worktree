...
class CommandLineEdit(QLineEdit):
    ...
    def event(self, a0): # the name `a0` is from baseclass
        ...
        if event.type() == QEvent.KeyPress and event.key() == Qt.Key_Tab: # type: ignore because we've checked it dynamically
    ...
    def start_completion(self):
        ...
        mcp, pcl = self.shell.auto_complete(incomplete_command)
        if mcp is None:
            return
        ...
        self.possible_completion_list = pcl
        self.set_current_argument(mcp)
