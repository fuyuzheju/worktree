...
class Notification:
    ...
    def send_notification(self, title, body, 
        ...
        """
        try:
            from winsdk.windows.ui.notifications import ToastNotificationManager, ToastNotification, ToastActivatedEventArgs # type: ignore
            from winsdk.windows.data.xml.dom import XmlDocument # type: ignore
            from winsdk.windows.foundation import IPropertyValue # type: ignore

        except ImportError:
            self.logger.debug('Package Not Found: winsdk')
            return

        if self.input_xml == '' and self.action_xml == '':
            self.logger.debug('Before send a notification, you need to add a category.')
        ...
        def activated(sender, args_obj):
            ...
            try:
                ...
                self.logger.error(f"Error processing activation: {e}")
                return
